...
/// @copydetails WS2812Controller800Khz
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB>
class SK6812Controller : public ClocklessController<DATA_PIN, 30 * FMUL, 30 * FMUL, 40 * FMUL, RGB_ORDER> {};
...
// WS2811 - 320ns, 320ns, 640ns
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB>
class WS2811Controller800Khz : public ClocklessController<DATA_PIN, C_NS(320), C_NS(320), C_NS(840), RGB_ORDER> {};
...
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB>
class WS2812Controller800Khz : public ClocklessController<DATA_PIN, C_NS(1250), C_NS(2625), C_NS(7375), RGB_ORDER> {};

// WS2811@400khz - 800ns, 800ns, 900ns
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB>
class WS2811Controller400Khz : public ClocklessController<DATA_PIN, C_NS(1800), C_NS(1800), C_NS(1900), RGB_ORDER> {};
...
/*
The original WS2812 datasheets that I had done this off of years ago had 
T0H as being 250ns, 
TOL 1µs and 
T1H being 875ns and 
T1L being 375ns give or take. 
The important part is to make sure the total timing is 1.25µs per bit which these timings are - the newer datasheets have different timings, 
however I'm not going to change the timings in the library until I get back to actively working on it and can do a full round of testing with different timings 
(the timings in here are based on hundreds of hours of writing/running tests and checking output w/scopes, etc...)

As for the three numbers - call them T1, T2, and T3. 
T0H is equal to T1, 
T0L is equal to T2+T3, 
T1H is equal to T1+T2,
T1L is equal to T3. 

T1 is the point in time where the signal goes from high to low for a zero. 
T1+T2 is the point in time where the signal always goes to low (if it's already low from being a zero, this doesn't change anything)
T1+T2+T3 is the total amount of time that must pass before starting the next bit. 
The timings are defined for how the code uses them.
*/
// ! value are calculated with this formula
/*
T1 = T0H
T2 = T0L - T1L
T3 = T1L
...
// new values
// 
/*  value from K32 library for LED_SK6812W_V3
 T0H = 350
 T1H = 700
 T0L = 800
 T1L = 600
 TRS = 50000
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB>
class SK6812Controller : public ClocklessController<DATA_PIN, C_NS(350), C_NS(200), C_NS(600), RGB_ORDER> {};
*/
...
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB>
class TM1934Controller : public ClocklessController<DATA_PIN, C_NS(100), C_NS(200), C_NS(300), RGB_ORDER> {};
